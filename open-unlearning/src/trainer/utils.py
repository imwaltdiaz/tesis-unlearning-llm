...
def compute_dpo_loss(model, ref_model, win_inputs=None, lose_inputs=None, beta=1.0):
    ...
    if win_inputs is not None:
        ...
        with torch.no_grad():
            win_ref_inputs = _move_inputs(win_inputs, ref_device)
            win_ref_loss, _ = compute_batch_nll(ref_model, win_ref_inputs)
            win_ref_loss = win_ref_loss.to(win_loss.device)
        win_log_ratio = -(win_loss - win_ref_loss)

    if lose_inputs is not None:
        lose_loss, lose_outputs = compute_batch_nll(model, lose_inputs)
        with torch.no_grad():
            lose_ref_inputs = _move_inputs(lose_inputs, ref_device)
            lose_ref_loss, _ = compute_batch_nll(ref_model, lose_ref_inputs)
            lose_ref_loss = lose_ref_loss.to(lose_loss.device)
