...
def plot_pareto_frontier(domain: str = "literatura", n_batches_list: list = None,
    ...
    Cada punto = un paso iterativo de un experimento (algo × n_batches).
    Los puntos dominantes (Pareto-óptimos) se resaltan con un borde negro.
    """
    ...
    all_points = []  # colección de (mu, fq) para calcular dominancia
    ...
    for algo in ALGORITHMS:
        for nb in n_batches_list:
            ...
            for s in steps_data:
                ...
                mus.append(float(mu))
                fqs.append(float(fq))
                all_points.append((float(mu), float(fq)))

            if not mus:
                continue

            label = f"{algo} (b={nb})"
            alpha = 0.5 + 0.15 * n_batches_list.index(nb)  # más opaco a más lotes
    ...
    ax.set_title(
        f"Frontera de Pareto: Forget Quality vs Model Utility\n"
    ...
    plt.tight_layout()
    out = output_dir / f"{domain}_pareto_frontier.png"
    plt.savefig(out, dpi=200, bbox_inches="tight")
    plt.close(fig)
    print(f"[OK] Frontera de Pareto → {out}")
